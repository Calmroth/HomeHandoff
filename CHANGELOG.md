...
- **Home is a welcome screen, not a control panel.** It used to stack seven equal-weight sections (Scenes, Lights, Music, Sound, Power, Sensors, Activity) always, whether or not anything was on — a wall of dark cards with no entry point for the eye. Now:
  ...
  - When idle, the hero names the **actual last-played playlist** and shows its real cover art (via Spotify's public oEmbed) — e.g. "Pick up where you left off · Today's Top Hits". The last-played selection now persists across reloads, which it never did before; without that the wall iPad forgot what you were listening to on every refresh. If nothing has ever been played it says so plainly rather than inventing a generic "Music".

### Changed
- **The backdrop now grades to the live weather.** The one daytime photo is overcast, so a clear sunny day used to render as grey fog — the backdrop contradicted the "Clear ☀" readout beside it. `<html>` now carries `data-weather` + `data-sky`, and CSS colour-grades the photo to match: a clear day gets warmed and brightened with a low sun-glow wash top-right; cloudy stays muted; rain goes cool and dim. The grade only touches the daytime photo — night and twilight keep their own shots. It can't paint blue sky over cloud (that needs a real sunny photo), but the canvas now reads warm on a sunny day and cold in the rain instead of always looking fogged in.
- **Sections are colour-coded by domain.** The dashboard was an amber monolith; now each section header carries its own accent — lights amber, sound teal, power green, climate blue, weather sky, sensors slate, activity coral — as both the title colour and a small leading dot. Derived automatically from each section's title/integration, so every page gets it without per-call wiring. Kept in the warm-dark family at moderate chroma so nothing turns garish.

### Fixed
- **Plejd now reflects lights it didn't switch itself.** The app only knew the state of lights it toggled in-app: a lamp lit by a wall switch (or already on when the hub connected) showed as off, because the BLE state-notification path was disabled and Plejd's cloud state is stale now that all control goes over Bluetooth. The hub re-subscribes to the mesh's state broadcasts, so wall-switch presses, scenes, and changes from the phone app now update the dashboard as the mesh reports them. The subscribe was originally removed because its CCCD write could knock a fresh WinRT link over; it's back defensively — deferred until the link settles, non-fatal on failure, and self-disabling to control-only if it destabilises that particular Bluetooth adapter, so control never regresses. (Caveat: a lamp already on before the hub connects, with no change since, may lag until it next reports — Plejd doesn't hand out a full state dump over BLE.)
- **Room filter labels were invisible over the photo.** The 21-room filter pills were muted-grey text on transparent with a hairline border, so any pill sitting over a bright part of the backdrop vanished. Each pill now has a dark blurred-glass backing and foreground-weight text, giving a consistent contrast floor regardless of what's behind it; the active pill reads as bright amber on amber-tinted glass.
- **`npm run hub` crashed with a raw stack trace when a hub was already running.** Starting a second hub (the most common by-hand mistake — the app already works because the first one is serving it) dumped an unhandled `EADDRINUSE` Node stack trace that read like a breakage. The hub now detects the occupied port up front (probing the same IPv6 stack it binds) and prints a plain-language "a hub is already running, you don't need another" notice, exiting 0. A WebSocket-server error guard prevents the same crash on the rare check-then-bind race.
- **The weather backdrop "clear vs cloudy" choice never existed.** `backdrop-day.avif` was a byte-identical copy of `backdrop-cabin.avif` (same SHA-256), so `pickBackdrop()` branched between two names for the same picture and the bundle shipped 111 KB of duplicate payload to do it. Collapsed to one overcast daylight photo serving both, duplicate deleted, and the reason recorded in the code so the split can be restored when a genuine sunlit exposure of the cabin exists.
- **The snow backdrop blew out the dark UI.** `backdrop-winter.avif` measured a mean luma of 228 where every other backdrop sits between 65 (sunset) and 155 (rain) — a near-white snowfield dropped into a warm-dark dashboard. Tone-mapped to 146, matching the cabin's 147, which also cut it from 13 KB to 7 KB. It remains the odd one out compositionally (it is a bare snowfield with no cabin, unlike the rest of the set) and wants regenerating.
