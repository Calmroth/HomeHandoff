// MusicPlayerWidget -- ported from 21st.dev "music-player-widget.tsx".
// TypeScript stripped; vanilla JSX. Stylesheet lives alongside in
// `musicPlayerWidget.css` and uses the clay/amber/motion tokens from
// `src/tokens.css` -- no Tailwind, no shadcn.
//
// Usage:
//   import MusicPlayer from './components/MusicPlayerWidget';
//   import './components/musicPlayerWidget.css';
//   <MusicPlayer tracks={tracks} crossOrigin="anonymous" />
//
// Track shape: { title, artist, cover, src }
...
/* ----------------------------------------------------------- ScalesMixer */
// Frequency-reactive 10x10 dot grid. Phase A bobs columns vertically; phase B
// scales each circle in/out. With audio data both motions are gain-boosted by
// per-band energy; without, it animates idle.
...
function Disc({
  ...
  return (
    ...
    >
      ...
      <div className="mpw-hole">
        <div className="mpw-hole-inner" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="mpw-track-info">
      {layers.map((l, i) => {
        ...
        const style = { '--mpw-dx': `${isNewest ? dx : exitDx}px` };
        return (
          <div
            key={l.id}
            className={`mpw-ti-layer ${isNewest ? '' : 'mpw-ti-abs'}`}
          >
            <p className={`mpw-artist ${state}`} style={style}>
              {l.track.artist}
            </p>
            <h2 className={`mpw-track ${state}`} style={style}>
              {l.track.title}
            </h2>
          </div>
        );
  ...
  return (
    <>
      ...
      <div className="mpw-time">
        <span className="mpw-time-current">{fmt(currentTime)}</span>
        <span className="mpw-time-sep">/</span>
        <span className="mpw-time-total">{fmt(duration)}</span>
      </div>
    </>
  );
}
...
function Controls({
  ...
  return (
    <div className="mpw-controls">
      ...
      <button
        className="mpw-ctrl mpw-ctrl-play"
        onClick={onToggle}
      ...
      <button
        className={`mpw-ctrl mpw-ctrl-toggle mpw-ctrl-loop ${
...
export function MusicPlayer({ tracks, crossOrigin }) {
  ...
  return (
    <div
      ...
      onClick={(e) => {
        if (!e.target.closest('.mpw-mask')) setIsZoomed(false);
      }}
    >
      <audio
        ref={player.audioRef}
